"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Node = void 0;
var Node = /** @class */ (function () {
    function Node(data) {
        this.name = data.name;
        this.ip = data.ip;
        this.port = data.port;
        this.website = data.website;
        this.guardianAddress = data.guardianAddress;
        this.nodeAddress = data.nodeAddress;
        this.reputation = data.reputation;
        this.online = false; // Will be updated when status is fetched
        this.effectiveStake = data.effectiveStake;
        this.enterTime = data.enterTime;
        this.weight = data.weight;
        this.inCommittee = data.inCommittee;
        this.teeHardware = data.teeHardware || false;
    }
    Node.prototype.updateStatus = function (timeoutMs) {
        if (timeoutMs === void 0) { timeoutMs = 5000; }
        return __awaiter(this, void 0, void 0, function () {
            var url, controller, timeoutId, response, statusData, e_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        url = "http://".concat(this.ip, ":").concat(this.port, "/services/management-service/status");
                        controller = new AbortController();
                        timeoutId = setTimeout(function () { return controller.abort(); }, timeoutMs);
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 6, 7, 8]);
                        return [4 /*yield*/, fetch(url, { signal: controller.signal })];
                    case 2:
                        response = _a.sent();
                        if (!response.ok) return [3 /*break*/, 4];
                        return [4 /*yield*/, response.json()];
                    case 3:
                        statusData = _a.sent();
                        this.online = true;
                        this.statusData = statusData;
                        this.updatedTime = Date.now();
                        return [3 /*break*/, 5];
                    case 4:
                        this.online = false;
                        this.updatedTime = Date.now();
                        _a.label = 5;
                    case 5: return [3 /*break*/, 8];
                    case 6:
                        e_1 = _a.sent();
                        this.online = false;
                        this.updatedTime = Date.now();
                        return [3 /*break*/, 8];
                    case 7:
                        clearTimeout(timeoutId);
                        return [7 /*endfinally*/];
                    case 8: return [2 /*return*/];
                }
            });
        });
    };
    Node.prototype.buildServiceUrl = function (path) {
        // Remove leading slash if present, then add it back to ensure consistent format
        var normalizedPath = path.startsWith('/') ? path : "/".concat(path);
        // If port is 0, omit it (use default HTTP port 80)
        var portPart = this.port === 0 ? '' : ":".concat(this.port);
        return "http://".concat(this.ip).concat(portPart, "/services").concat(normalizedPath);
    };
    Node.prototype.get = function (path, timeoutMs) {
        if (timeoutMs === void 0) { timeoutMs = 5000; }
        return __awaiter(this, void 0, void 0, function () {
            var url, controller, timeoutId, response;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        url = this.buildServiceUrl(path);
                        console.log("Node GET ".concat(url));
                        controller = new AbortController();
                        timeoutId = setTimeout(function () { return controller.abort(); }, timeoutMs);
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, , 3, 4]);
                        return [4 /*yield*/, fetch(url, {
                                method: 'GET',
                                signal: controller.signal
                            })];
                    case 2:
                        response = _a.sent();
                        return [2 /*return*/, response];
                    case 3:
                        clearTimeout(timeoutId);
                        return [7 /*endfinally*/];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    Node.prototype.post = function (path, body, timeoutMs) {
        if (timeoutMs === void 0) { timeoutMs = 5000; }
        return __awaiter(this, void 0, void 0, function () {
            var url, controller, timeoutId, headers, requestBody, response;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        url = this.buildServiceUrl(path);
                        controller = new AbortController();
                        timeoutId = setTimeout(function () { return controller.abort(); }, timeoutMs);
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, , 3, 4]);
                        headers = {};
                        requestBody = void 0;
                        if (body !== undefined) {
                            if (typeof body === 'string') {
                                requestBody = body;
                                headers['Content-Type'] = 'text/plain';
                            }
                            else {
                                requestBody = JSON.stringify(body);
                                headers['Content-Type'] = 'application/json';
                            }
                        }
                        return [4 /*yield*/, fetch(url, {
                                method: 'POST',
                                headers: headers,
                                body: requestBody,
                                signal: controller.signal
                            })];
                    case 2:
                        response = _a.sent();
                        return [2 /*return*/, response];
                    case 3:
                        clearTimeout(timeoutId);
                        return [7 /*endfinally*/];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    return Node;
}());
exports.Node = Node;
//# sourceMappingURL=node.js.map